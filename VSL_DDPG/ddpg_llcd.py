...
def from_a_to_mlv(a):
   return 17.8816 + 2.2352*np.floor(a)   #floor向下取整  40miles/h+5*np.floor(a)
    #return 15 + 3*np.floor(a)   #floor向下取整  40miles/h+5*np.floor(a)
...
for ep in range(EP_MAX):
    ...
    while simulationSteps < 3600:
        ...
        v = from_a_to_mlv(a)
        all_v = np.vstack((all_v,v))##v=40+5*a
        stime_ = np.zeros(25,)    #state清零
        s_, r, simulationSteps, oflow_temp, bspeed_temp = net.run_step(v)    ##新的v代入
...
#            traci.vehicle.setLaneChangeMode(vid_list[i], 0b001000000000)
        ...
        if total_step > MEMORY_CAPACITY:
            var = abs(1.5 - 1.5/300*ep)    # decay the action randomness
            vsl_controller.learn()
        stime = stime_
        ep_r += r
    all_ep_r.append(ep_r)
    # print(simulationSteps,all_v)
    all_oflow.append(oflow)
    all_bspeed.append(bspeed/3600)
    net.close()
    ...
    with open(laneData, 'r') as f:  # 打开文件
        lines = f.readlines() # 读取所有行
        for lane in range(3):
            mainlane= lines[lane+54]  # 取gne1那三行
            ...
            print('mainlane',lane,': traveltime=',lane_traveltime,'waitingTime=',lane_waitingTime,'speed=',lane_speed)
    print('Episode:', ep, ' Rewards: %.4f' % ep_r,\
          'Bottleneck speed: %.4f' % bspeed, 'out-in flow: %.4f' % oflow, 'Average travel time: %.4f' % aat_tempo)
    if all_ep_r[ep] == max(all_ep_r) :
        vsl_controller.savemodel()
        print('----------------------------save model-----------------------------, episode=', ep)
        np.savetxt('vsl_control.txt',all_v,fmt='%.04f')
        # f = xlwt.Workbook()
        # sheet1 = f.add_sheet('speed', cell_overwrite_ok=True)  # 创建sheet
        # #row0 = ["Id", "Type", "Distance", "0%", "100%"]
        # for i in range(len(all_v)):
        #     sheet1.write(0, i, all_v[i])
        # f.save('vsl_control.xls')  # 保存文件
    time_end=time.time()
    print('totally cost',time_end-time_start)
vsl_controller.plot_reward()
