...
void TraCICommandInterface::Vehicle::setTypeId(std::string TypeId)
{
    uint8_t variableId = VAR_TYPE;
    uint8_t variableType = TYPE_STRING;
    TraCIBuffer buf = traci->connection.query(CMD_SET_VEHICLE_VARIABLE, TraCIBuffer() << variableId << nodeId << variableType << TypeId);
    ASSERT(buf.eof());
}
...
// by FTT 4/3
void TraCICommandInterface::Lane::setMaxSpeed(double speed)
{
    uint8_t variableId = VAR_MAXSPEED;
    uint8_t variableType = TYPE_DOUBLE;
    TraCIBuffer buf = traci->connection.query(CMD_SET_LANE_VARIABLE, TraCIBuffer() << variableId << laneId << variableType << speed);
    ASSERT(buf.eof());
}
//
